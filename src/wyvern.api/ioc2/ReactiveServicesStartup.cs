...
using wyvern.api.@internal.utils;
...
namespace wyvern.api.ioc2
{
    public class ReactiveServicesStartup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddCors()
                .AddRouting()
                .AddSingleton<Config>(x => new ConfigurationLoader().Load())
                .AddSingleton(x => new ReadSideConfig(x.GetService<Config>()))
                .AddSingleton<ActorSystemLifetime>()
                // TODO: Only load this if we need it via config...
                .AddSingleton<ReadSide, ReadSideImpl>() 
                // TODO: Cascade configs.
                .AddSingleton(x => x.GetService<ActorSystemLifetime>().CreateActorSystem());
